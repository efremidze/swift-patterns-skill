...
**Primary recommendation:** Make `swift-expert-skill/SKILL.md` the authoritative entry point with required YAML frontmatter, a single constraints section, and refactor/review response templates that explicitly enforce the citations allowlist in `swift-expert-skill/references/`.
...
**Warning signs:** Citations to external URLs not found in `swift-expert-skill/references/`.
...
1. **Citation allowlist contents**
   ...
   - What's unclear: Which authoritative URLs should be added to `swift-expert-skill/references/*.md` as the allowlist.
